...
- **Routes** (wouter): `/` (Home), `/about` (About), `/music` (full discography), `*` (NotFound).
...
- **/music page** (`src/pages/Music.tsx`): full discography organized by artist with filter chips (all / per-artist). Each release card has cover, tag chip, title, optional year, and links externally to its streaming page. All in-app "listen" CTAs (carousel tiles, roster tiles, "listen all", "all releases", nav "music" link) navigate here instead of going out to lnk.to. Catalog data lives in `src/data/catalog.ts` (`ARTISTS` grouped by slug `eric-benet`, `joe-leone`, `autumn-paige`).
